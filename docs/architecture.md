...
| **ui/** | `statusBar.ts`, `sessionsView.ts`, `nowView.ts` | Status bar (live duration + description), tree view (sessions grouped by day with per-session detail rows), pinned "Now" footer (clock + current session, ticks every second) |
